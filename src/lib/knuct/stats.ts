...
export async function getKnuctDashboardStats(): Promise<KnuctDashboardStats> {
  ...
  const [total, active, failed, pending, userCount] = await Promise.all([
  ...
  const didCoveragePct =
    userCount > 0 ? Math.round((active / userCount) * 100) : 0;
