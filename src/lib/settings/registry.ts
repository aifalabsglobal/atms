...
const defs: SettingDefinition[] = [
  ...
  // Organization
  {
    ...
    subcategory: 'Calendar week',
    displayName: 'First day of week',
    description:
      'Which weekday starts the campus calendar month grid and week headers. Most campuses in India use Monday.',
    valueType: 'enum',
    defaultValue: 1,
    validation: {
  ...
  },
  {
    ...
    description:
      'Weekdays Mon–Fri (and Sun if selected). Saturday is controlled separately by Saturday mode.',
    valueType: 'json',
  ...
  },
  {
    ...
    category: 'organization',
    subcategory: 'Working time',
    displayName: 'Block marking on holidays & non-working days',
    description:
      'When On, self-mark uses Calendar holidays, compensatory working days, exam-day policy, and Working days.',
    valueType: 'boolean',
    defaultValue: false,
  },
  {
    key: 'organization.exam_day_attendance',
    category: 'organization',
    subcategory: 'Working time',
    displayName: 'Exam-day attendance',
    description:
      'How self-mark behaves on Calendar “Examination” days when holiday/non-working blocking is On.',
    valueType: 'enum',
    defaultValue: 'allowed',
    validation: {
      allowedValues: ['allowed', 'blocked', 'optional'],
      optionLabels: {
        allowed: 'Allowed (normal working-day rules)',
        blocked: 'Blocked',
        optional: 'Optional (allow even on weekends)',
      },
    },
  },
  {
    key: 'organization.day_start_time',
    category: 'organization',
    subcategory: 'Working time',
    displayName: 'Campus day starts',
    description: 'Official campus open time (24h HH:mm). Used when “Enforce campus day hours” is On.',
    valueType: 'string',
    defaultValue: '08:00',
    validation: { regex: '^([01]\\d|2[0-3]):[0-5]\\d$' },
  },
  {
    key: 'organization.day_end_time',
    category: 'organization',
    subcategory: 'Working time',
    displayName: 'Campus day ends',
    description: 'Official campus close time (24h HH:mm). Used when “Enforce campus day hours” is On.',
    valueType: 'string',
    defaultValue: '17:00',
    validation: { regex: '^([01]\\d|2[0-3]):[0-5]\\d$' },
  },
  {
    key: 'organization.saturday_mode',
    category: 'organization',
    subcategory: 'Working time',
    displayName: 'Saturday mode',
    description:
      'How Saturdays count as working days. Overrides the Saturday checkbox in Working days. Alternate = odd ISO weeks only.',
    valueType: 'enum',
    defaultValue: 'off',
    validation: {
      allowedValues: ['full', 'half', 'off', 'alternate'],
      optionLabels: {
        full: 'Full working day',
        half: 'Half day (ends at half-day time)',
        off: 'Off (non-working)',
        alternate: 'Alternate Saturdays (odd ISO weeks)',
      },
    },
  },
  {
    key: 'organization.half_day_end_time',
    category: 'organization',
    subcategory: 'Working time',
    displayName: 'Half-day ends',
    description: 'Close time on half-day Saturdays (24h HH:mm). Used when Saturday mode is Half day.',
    valueType: 'string',
    defaultValue: '13:00',
    validation: { regex: '^([01]\\d|2[0-3]):[0-5]\\d$' },
  },
  {
    key: 'organization.enforce_day_hours',
    category: 'organization',
    subcategory: 'Working time',
    displayName: 'Enforce campus day hours',
    description:
      'When On, self-mark attendance is rejected outside Campus day starts–ends (campus timezone).',
    valueType: 'boolean',
    defaultValue: false,
  },
  {
    key: 'organization.period_minutes',
    category: 'organization',
    subcategory: 'Timetable',
    displayName: 'Default period length',
    description: 'Minutes per teaching period. Used as the default slot duration when adding timetable slots.',
    valueType: 'number',
    defaultValue: 50,
    validation: { min: 15, max: 240 },
  },
  {
    key: 'organization.break_minutes',
    category: 'organization',
    subcategory: 'Timetable',
    displayName: 'Default break between periods',
    description: 'Minutes between periods in the suggested campus day schedule.',
    valueType: 'number',
    defaultValue: 10,
    validation: { min: 0, max: 120 },
  },
  {
    key: 'organization.periods_per_day',
    category: 'organization',
    subcategory: 'Timetable',
    displayName: 'Periods per day',
    description: 'How many teaching periods fit in a normal campus day (for schedule preview and defaults).',
    valueType: 'number',
    defaultValue: 6,
  ...
  },
  {
    ...
    category: 'organization',
    subcategory: 'Academic rules',
    displayName: 'Require an active academic year',
    description:
      'When On, Masters and Calendar warn if no academic year is marked active, and subject publish is blocked until one is set.',
    valueType: 'boolean',
    defaultValue: true,
  },
  {
    key: 'organization.allow_multiple_active_years',
    category: 'organization',
    subcategory: 'Academic rules',
    displayName: 'Allow multiple active academic years',
    description:
      'When Off (recommended), marking a year Active automatically marks other active years as Completed.',
    valueType: 'boolean',
    defaultValue: false,
  },
  {
    key: 'organization.default_regulation',
    category: 'organization',
    subcategory: 'Academic rules',
    displayName: 'Default regulation code',
    description: 'Pre-filled regulation for new academic years (e.g. R22, R25).',
    valueType: 'string',
    defaultValue: 'R22',
  },
  {
    key: 'organization.lock_completed_academic_years',
    category: 'organization',
    subcategory: 'Academic rules',
    displayName: 'Lock completed academic years',
    description:
      'When On, Admins cannot edit or delete Completed years. Super Admin can still override.',
    valueType: 'boolean',
    defaultValue: true,
  },
  {
    key: 'organization.require_semester_for_publish',
    category: 'organization',
    subcategory: 'Academic rules',
    displayName: 'Require semester under active year',
    description:
      'When On, subject publish and timetable slots need a semester linked to the active academic year.',
    valueType: 'boolean',
    defaultValue: false,
  },
  {
    key: 'organization.auto_promote_academic_year',
    category: 'organization',
    subcategory: 'Academic rules',
    displayName: 'Auto-promote upcoming academic year',
    description:
      'When On, a daily job promotes the earliest Upcoming year to Active once its start date arrives.',
    valueType: 'boolean',
    defaultValue: false,
  },
  {
    key: 'organization.campus_code',
    category: 'organization',
    subcategory: 'Campus identity',
    displayName: 'Campus code',
    description: 'Short campus / institute code shown on exported reports (e.g. AIMSCS).',
    valueType: 'string',
    defaultValue: '',
  },
  {
    key: 'organization.aishe_code',
    category: 'organization',
    subcategory: 'Campus identity',
    displayName: 'AISHE / UGC ID',
    description: 'AISHE or UGC identifier for compliance and report letterheads.',
    valueType: 'string',
    defaultValue: '',
  },
  {
    key: 'organization.campus_address',
    category: 'organization',
    subcategory: 'Campus identity',
    displayName: 'Campus address',
    description: 'Postal address for report headers (logo and institute name stay under General).',
    valueType: 'string',
    defaultValue: '',
  },
  {
    key: 'organization.campus_phone',
    category: 'organization',
    subcategory: 'Campus identity',
    displayName: 'Campus phone',
    description: 'Main contact number printed on exported reports.',
    valueType: 'string',
    defaultValue: '',
  },
  {
    key: 'organization.principal_title',
    category: 'organization',
    subcategory: 'Campus identity',
    displayName: 'Principal / Director title',
    description: 'Signatory line for documents (e.g. Director, C.R. Rao AIMSCS).',
    valueType: 'string',
    defaultValue: '',
  },
